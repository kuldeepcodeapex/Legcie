...
const {  establishmentid, setEstablishmentid,setdata, data,signOut,establishmentscount,setEstablishmentscount}: any = useAuth()
...
const [search,setSearch]=useState()
     ...
     const fetchdata = async () => {
        ...
        setFinaldata(data)
        setdata(data)
        setEstablishmentscount(data?.length)
    }


let searchEstablishments=()=>{
    if(search){
 let a =  finaldata?.filter((v)=> 
(search==v.name||search==v.email)
)
return a
...
let filterResult =()=> finaldata?.filter((v)=> 
(v.subscription==(checkPlan?.label))
)
useEffect(()=>{
    let obj=searchEstablishments()
    setdata(obj)
...
useEffect(()=>{
    let obj=filterResult()
    setdata(obj)
    if("All plans"==(checkPlan?.label)){
        setdata(fetchdata)
    }
    setSearch('')
    ...
    return (
        ...
        <View style={{ backgroundColor: "rgb(245, 241, 228)", height: "100%",  paddingBottom: 50, paddingHorizontal: 20 }}>
            <View style={{ width: "100%", alignItems: "center", gap: 10 }}>
                ...
                <Text>{establishmentscount} establishments in total</Text>
            ...
            <View style={{ backgroundColor: "white", marginVertical: 20, borderRadius: 10 ,gap:20}}>
                <View style={{ borderWidth: 1, borderRadius: 10, flexDirection: "row", alignItems: "center", paddingHorizontal: 10,margin:10 }}>
                    ...
                    <TextInput placeholder='Search for an establishments....' value={search} onChangeText={(v)=>{
                        setSearch(v)
                    }}>
                ...
                <View style={{ justifyContent: "center", alignItems: "center", flexDirection: "row", gap: 10,marginVertical:30 }}>
